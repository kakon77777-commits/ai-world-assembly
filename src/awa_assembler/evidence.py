...
from .common import AssemblerError, slug, validate
...
def validation_documents(root: Path, task: dict[str, Any], payload: bytes,
    ...
    for validator_id in task["validators"]:
        result = VALIDATORS[validator_id](payload)
        status = "passed" if result.passed else "failed"
        statuses.append(status); diagnostics.extend(result.diagnostics)
        vid = f"validation.audio.crystal_filterer.attack.{slug(validator_id)}.{artifact_sha[:12]}"
        doc = {
            "contract": "artifact-validation.v0.1", "validation_id": vid,
            "artifact_id": artifact_id, "artifact_sha256": artifact_sha,
            "validator": validator_id, "validator_version": "v0.1",
            "status": status, "diagnostics": result.diagnostics,
...
def artifact_reference(root: Path, producer_id: str, task: dict[str, Any], payload: bytes,
    ...
    aid = f"artifact.audio.crystal_filterer.attack.candidate.{artifact_sha[:16]}"
    doc = {
        "contract": "artifact-reference.v0.1", "artifact_id": aid,
        "kind": task["target"]["kind"], "uri": uri, "sha256": artifact_sha,
        "mime_type": task["target"]["mime_type"], "size": len(payload),
        "generator": producer_id, "license_ref": "license:internal-original",
        "parent_artifacts": [], "validation_status": validation_status,
    }
...
def preview_graph(root: Path, task: dict[str, Any], generation: dict[str, Any],
    ...
    target = {
        "contract": "asset-graph-node.v0.1", "node_id": task["target"]["node_id"],
        "node_type": "artifact", "namespace": "game.alien_lineage",
        "semantic_ref": generation["target"]["node_ref"], "version": task["target"]["version"],
        "status": "candidate",
        "metadata": {"artifact_ref": artifact["artifact_id"], "assembler_task": task["task_id"], "role": "creature_attack_sfx"},
    ...
    nodes[target["node_id"]] = target; artifacts[artifact["artifact_id"]] = artifact
    vnodes, vedges = [], []
    for item in validations:
        validation_registry[item["validation_id"]] = item
        node = {
            "contract": "asset-graph-node.v0.1", "node_id": item["validation_id"],
            "node_type": "validation", "namespace": "game.alien_lineage",
            "semantic_ref": generation["target"]["node_ref"], "version": "v0.1", "status": "active",
            "metadata": {"validation_ref": item["validation_id"]},
            "provenance_ref": f"assembler-validation://{item['validation_id']}",
        }
        edge = {
            "contract": "asset-graph-edge.v0.1", "edge_id": f"edge:{target['node_id']}->{item['validation_id']}",
            "from": target["node_id"], "to": item["validation_id"], "type": "validated_by",
            "required": False, "version_constraint": None, "scope": "validation",
            "reason": "Phase 8 exact-byte candidate validation evidence",
        ...
        nodes[node["node_id"]] = node; edges[edge["edge_id"]] = edge
        vnodes.append(node); vedges.append(edge)
...
def run_receipt(root: Path, task: dict[str, Any], source_hashes: dict[str, str],
    ...
    if selected and preview:
        ...
        graph = {"snapshot_id": preview["snapshot_id"], "sha256": preview["sha256"],
        ...
        promotion = {"status": "validated_candidate", "target_node_status": "candidate", "canonical_write": False,
        ...
        promotion = {"status": "blocked", "target_node_status": "none", "canonical_write": False,
                     "reason": "attempt budget exhausted before a build-complete validated candidate was produced"}
    basis = {"task_id": task["task_id"], "status": status, "source_hashes": source_hashes,
             "attempts": attempts, "selected_candidate": chosen, "graph_preview": graph, "promotion": promotion}
